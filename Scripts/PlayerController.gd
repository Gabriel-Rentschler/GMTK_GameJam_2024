...
@export_range(0.01,1,0.01) var JUMP_THROTTLE : float # 0.1 # Stores preference for time before the player can jump again
...
### Groundedness raycasts
	...
	for array in raycast_list:
		var direct_query_parameters = PhysicsRayQueryParameters3D.create(array[0],array[1], 0x00000005,
[self])
...
func move(move,state):
	if is_grounded:
		...
		var direct_query_parameters = PhysicsRayQueryParameters3D.create(start, end, 0x00000005,
[self])
		...
		if (contacted_body != null):
			contacted_body.apply_force(move * -accel,state.get_contact_collider_position(0))
	else:
